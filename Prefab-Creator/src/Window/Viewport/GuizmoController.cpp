...
namespace Tempest
{
	...
	void GuizmoController::Draw(Camera& cam, tc::Local& transform)
	{
		if (m_IsEnabled)
		{
			Set(cam, transform);
	...
	void GuizmoController::Set(Camera& cam, tc::Local& form)
	{
		auto vp = cam.GetViewport();
		vec2 Min = { 0, 0 };
		vec2 Max = { vp.z, vp.w };
		...
		auto mat =
			glm::translate(glm::make_vec3(value_ptr(form.local_position)))
			* glm::mat4(form.local_rotation)
			* glm::scale(glm::make_vec3(value_ptr(form.local_scale)));

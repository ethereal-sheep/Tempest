...
void EmitterSystem_2D::CreateButtonEmitter(std::weak_ptr<WaypointEmitter_2D>& emitter, glm::vec2 topLeftPos, glm::vec2 buttonSize)
{
	...
	emitter.lock()->m_PAM.m_startVelocity = glm::vec2{ 0.f, 0.f };
	emitter.lock()->m_PAM.m_endVelocity = glm::vec2{ 0.f, 0.f };
	emitter.lock()->m_PAM.m_velocityVariation = glm::vec2{ 0.0f, 0.0f };

	emitter.lock()->m_PAM.m_sizeBegin = 3.0f;
	emitter.lock()->m_PAM.m_sizeEnd = 3.0f;
	emitter.lock()->m_PAM.m_sizeVariation = 0.0f;
...
void EmitterSystem_2D::CreateExplosionEmitter(std::weak_ptr<ExplosionEmitter_2D>& emitter, glm::vec2 spawnPos)
{
	...
	emitter.lock()->m_PAM.m_sizeBegin = 10.0f;
	emitter.lock()->m_PAM.m_sizeEnd = 0.0f;
	emitter.lock()->m_PAM.m_sizeVariation = 0.3f;
...
void EmitterSystem_2D::CreateCircularMotionEmitter_2D(std::weak_ptr<CircularMotionEmitter_2D>& emitter, glm::vec2 centrePos, float radius)
{
	...
	emitter.lock()->m_PAM.m_sizeBegin = 8.0f;
	emitter.lock()->m_PAM.m_sizeEnd = 8.0f;
	emitter.lock()->m_PAM.m_sizeVariation = 0.0f;
...
//void EmitterSystem_2D::ReuseCircularMotionEmitter_2D(const std::shared_ptr<CircularMotionEmitter_2D>& emitter, ImVec2 centrePos, float radius)
//{
//	ReuseCircularMotionEmitter_2D(emitter, Imvec2_To_GlmVec2_Converter(centrePos), radius);
//}
//
//void EmitterSystem_2D::ReuseCircularMotionEmitter_2D(const std::shared_ptr<CircularMotionEmitter_2D>& emitter, glm::vec2 centrePos, float radius)
//{
//	// Center position of the circle

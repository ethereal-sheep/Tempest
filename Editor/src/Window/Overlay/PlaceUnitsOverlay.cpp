...
namespace Tempest
{
	...
	void PlaceUnitsOverlay::show(Instance& instance)
	{
		...
		if (OverlayOpen)
		{
			...
			if (ImGui::Begin("Place Units Overlay", nullptr, window_flags))
			{
				...
				window->AddRectFilled({ sideBgMin.x, sideBgMin.y + ImGui::GetWindowHeight() * 0.1f }, { sideBgMin.x + imgWidth - 10.f,  sideBgMin.y + ImGui::GetWindowHeight() * 0.1f + strSize.y  }, ImGui::GetColorU32(col));
				
				ImGui::PushFont(FONT_TURN);
				window->AddText({ sideBgMin.x, sideBgMin.y + ImGui::GetWindowHeight() * 0.1f }, ImGui::GetColorU32({ 0,0,0,1 }), str.c_str());
				ImGui::PopFont();

				ImGui::PushFont(FONT_BODY);
				string str2 = "Place your selected units into the level";
				window->AddText({ sideBgMin.x, sideBgMin.y + ImGui::GetWindowHeight() * 0.15f }, ImGui::GetColorU32({ 1,1,1,1 }), str2.c_str());
				ImGui::PopFont();
	...
	{
		if (ImGui::BeginChild("##PlaceUnitSideBar", size, true))
		{
